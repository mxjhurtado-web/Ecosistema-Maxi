...
async def google_chat_notify_handler_inner(
    ...
    if not target_space and request.destino:
        ...
        if destino_lower == "alertas":
            ...
            target_space = os.getenv("GOOGLE_CHATS_BSA_SPACE") or "spaces/AAQA3WL2JIk"
        else:
            target_space = settings.GOOGLE_CHATS_DEFAULT_SPACE
...
# ============================================================
# Debug Endpoint: Direct Read of Google Docs & Sheets via SA
# ============================================================

@router.get("/debug/read-google-sources")
async def debug_read_google_sources(
    secret: Optional[str] = Query(None),
    x_webhook_secret: Optional[str] = Header(None, alias="X-Webhook-Secret")
):
    """
    Directly reads and returns content from Google Doc Governance, Reglas RNE, and Scripts SC
    ...
    try:
        ...
        results["doc_governance_text"] = doc_text[:20000] if doc_text else "No content returned or permission denied"
    except Exception as e:
        results["doc_governance_error"] = str(e)

    # 2. Read Google Sheets Reglas (1eFm3L_ALVr78wTDBB2bsg7Wq6DT9ZoGzIX9tKLN9nGw) & Scripts (18VE3tdVt4E-eNrf0dD4zlk1aLV2nfv9_ncdUvLPaNic)
    try:
        from .google_chat_service import google_chat_service
        config = await config_manager.get_google_chat_config()
