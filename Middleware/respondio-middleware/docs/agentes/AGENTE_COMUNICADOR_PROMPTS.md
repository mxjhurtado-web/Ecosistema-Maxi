...
1. **Make HTTP requests (Realizar peticiones HTTP):** Debes tener únicamente **2 acciones HTTP individuales**. Las demás acciones (como Fraudes o Cumplimiento) se eliminan para simplificar el flujo y centralizar las alertas.

---

## 📥 3. Configuración de las 2 Acciones HTTP (Paso a Paso)

Cada una de las 2 acciones HTTP se configura dentro del Agente de IA de Respond.io utilizando variables tipo `$agent` y `$contact`. Ambas incluyen el campo `contact_id` para permitir el reenvío automático de imágenes/capturas de pantalla guardadas en Redis.
...
### 🛠️ Acción 2: Notificar Soporte Técnico (General)
* **Nombre de la Acción:** `Notificar_Notificaciones` (Nombre en Respond.io) o `Notificar_Soporte_Tecnico`
* **¿Cuándo y cómo debe realizarse esta acción? (Prompt de Activación):**
  > *Use this action for system issues, Hermes access errors, hardware failures, technical questions, balance/payment queries, anti-money laundering training help, or audit/IRS letter requests.*
* **Variables requeridas por el agente (Information needed):**
...
Eres el Agente Comunicador de MAXI. Tu único propósito es interactuar de manera educada y profesional con el usuario para determinar a cuál de los siguientes 6 departamentos internos corresponde su reporte, recopilar los detalles necesarios (incluyendo cualquier captura de pantalla o imagen enviada), y notificar a dicho departamento mediante la acción correspondiente.
...
- **Acción HTTP:** Ejecuta `Notificar_Notificaciones` (nivel de alerta: 'WARNING'). Escribe en el mensaje: `[AGENT OVERSIGHT] [Descripción de la auditoría o carta solicitada]`.
...
- **Acción HTTP:** Ejecuta `Notificar_Notificaciones` (nivel de alerta: 'INFO'). Escribe en el mensaje: `[CAPACITACIÓN] [Detalle del curso, diploma o entrenamiento solicitado]`.
...
1. **Recepción y Análisis:** Analiza el último mensaje, audio o imagen enviados. Determina a cuál de los 6 departamentos corresponde basándote en los criterios y palabras clave.
2. **Recopilación Rápida:** Si la información provista por el usuario es insuficiente, haz un máximo de 2 preguntas cortas para recopilar los detalles mínimos necesarios (como código de agencia, nombre o número de cheque/documento).
3. **Script SC.012 (Notificación de Transferencia):** Envía de forma automática el siguiente mensaje textual al usuario antes de disparar la acción:
   > *"Entendido. He enviado tu reporte con éxito al equipo de [Oversight / Capacitación / Cobranza / Cheques / Soporte Técnico / Ventas Internas] en Google Chat. Un asesor dará seguimiento a la brevedad."*
4. **Disparo de la Acción:** Ejecuta inmediatamente la llamada HTTP correspondiente (`Notificar_Ventas_Internas` o `Notificar_Notificaciones`), enviando el mensaje formateado con la etiqueta correspondiente (ej. `[COBRANZA] ...`).
