...
def _normalize_date_to_mdy_ctx(s: str, country_ctx: str|None, lang_ctx: str|None):
    ...
    # REGLA: Si sólo incluye año (plausible), devuélveme 12/31/YYYY
    # (Evita falsos positivos como "0805" que suelen ser parte de números de identificación)
    if re.fullmatch(r'\b\d{4}\b', st):
        try:
            y_int = int(st)
            if 1900 <= y_int <= 2099:
                return f"12/31/{y_int:04d}"
        except ValueError:
            pass
    # 🆕 FASE 1.1: MM/YYYY o MM-YYYY -> último día del mes (ej. 03/2027 -> 03/31/2027)
    ...
    if m:
        ...
        try:
            ...
            y_int = int(y)
            # Validar rangos
            if 1 <= mm_int <= 12 and 1900 <= y_int <= 2099:
                import calendar as _cal
                last_day = _cal.monthrange(y_int, mm_int)[1]
                return f"{mm_int:02d}/{last_day:02d}/{y_int:04d}"
        except ValueError:
            pass
...
def _process_all_dates_by_type(texto: str) -> Dict[str, Optional[str]]:
    ...
    vigencia_keywords = ["vencimiento", "vence", "expiración", "expiracion", "vigencia", "valido hasta", "válido hasta", "valid thru", "valid through", "valid until", "caducidad", "fecha de caducidad", "expires", "expire", "expiry", "expiration", "date of expiry", "date of expiration"]
    ...
    def process_and_assign_date(original: str, date_type: str):
        ...
        # Regla: Vigencia/Caducidad/Expiración con SOLO AÑO -> siempre "Sugerida"
        # (ej. "2029" -> "12/31/2029 (Sugerida)")
        if date_type == "vigencia":
            ...
            if rm:
                y_str = rm.group(2)
            elif re.fullmatch(r"\d{4}", o):
                y_str = o
            if y_str:
                try:
                    yi = int(y_str)
                    if 1900 <= yi <= 2099 and re.fullmatch(r"\d{2}/\d{2}/\d{4}", (final_date or "").strip()):
                        if "Sugerida" not in (final_date or ""):
    ...
    if not results["fecha_vigencia_final"]:
        # Intento 1: buscar en líneas relacionadas con vigencia/caducidad/expiración
        for line in texto.splitlines():
            ll = line.lower()
            if any(k in ll for k in vigencia_keywords):
                # 1) Intentar extraer y normalizar una fecha completa desde la misma línea
                #    (evita caer en sugerencias por detectar solo un año)
                candidates = _extract_all_dates(line)
                chosen = None

                # Prioridad A: fechas completas (con día)
                for cand in candidates:
                    cand_str = (cand or "").strip()
                    if not cand_str:
                        continue
                    # Saltar candidatos "solo año" o rango: se manejan después
                ...
                # Prioridad B: MM/YYYY (ej. 03/2027)
                if not chosen:
                    for cand in candidates:
                        cand_str = (cand or "").strip()
                        if not cand_str:
                ...
                # Prioridad C: rango de años o solo año -> sugerida 12/31/YYYY
                if not chosen:
                    range_match = _DATE_RE_YEAR_RANGE.search(line)
                    year_match = re.search(r"\b(19\d{2}|20\d{2})\b", line)
                    if range_match:
                        y_str = range_match.group(2)
                        base = _normalize_date_to_mdy_ctx(y_str, doc_pais, lang)
                        if base:
                            chosen = f"{base} (Sugerida)"
                    elif year_match:
                ...
                if chosen:
                    if "Sugerida" in chosen:
                        results["vigencia_sugerida"] = chosen
                    results["fecha_vigencia_final"] = chosen
                    break

        # Intento 2: si NO se encontró nada, sugerir hoy + 5 años (Sugerida)
        if not results["fecha_vigencia_final"]:
            try:
...
def _authenticity_score(texto: str, image_path: str|None):
    """
    Semáforo de autenticidad (solo autenticidad, no validez/expiración).

    Reglas de salida:
    - Riesgo ALTO  (🔴): evidencia fuerte de falsificación / patrón claramente inválido.
    - Riesgo MEDIO (🟡): hay sospecha o falta de información para asegurar autenticidad.
    - Riesgo BAJO  (🟢): no se detectan señales relevantes de falsificación.
    ...
    # Flags para decisión (evita falsos positivos por OCR incompleto)
    hard_fail = False           # Alta probabilidad / evidencia fuerte de falsificación
    suspect = False             # Sospecha (no concluyente) o información insuficiente
    strong_flags = 0            # Conteo de inconsistencias fuertes (para escalar a alto si se acumulan)

    def _add_user(msg: str):
        if msg and msg not in details_user:
            details_user.append(msg)

    def _add_internal(msg: str):
        if msg and msg not in details_internal:
            details_internal.append(msg)
    ...
    # 1. Chequeo de Muestra/Plantilla (evidencia fuerte)
    if any(w in low for w in _SAMPLE_WORDS if w):
        score += 60
        hard_fail = True
        _add_internal("⚠️ CRÍTICO: Contiene 'sample/muestra/void'")
        _add_user("Documento de muestra detectado")

    # 2. Chequeo de Nombre (si falta: no concluyente -> MEDIO)
    nombre = _extract_name(texto)
    if not nombre:
        score += 15
        suspect = True
        _add_internal("⚠️ No se detectó nombre válido")
        _add_user("Información incompleta")
    ...
    if dob_use and "Sugerida" not in dob_use:
        ...
        if age is not None and (age < 15 or age > 120):
            ...
            suspect = True
            strong_flags += 1
            _add_internal(f"⚠️ Edad implausible: {age} años")
            _add_user("Inconsistencia en datos personales")
        ...
        if curp_m and doc_pais == "MX":
            ...
            if curp_dob and curp_dob != dob_use:
                ...
                suspect = True
                strong_flags += 1
                _add_internal(f"⚠️ CURP no coincide: CURP={curp_dob} vs DOB={dob_use}")
                _add_user("Inconsistencia en identificadores oficiales")
        ...
        if rfc_m and doc_pais == "MX":
            ...
            if rfc_dob != dob_use:
                ...
                suspect = True
                strong_flags += 1
                _add_internal(f"⚠️ RFC no coincide: RFC={rfc_dob} vs DOB={dob_use}")
                _add_user("Inconsistencia en datos fiscales")
    else:
        # Falta DOB: incertidumbre (no afirmar falso)
        score += 8
        suspect = True
        _add_internal("⚠️ No se identificó fecha de nacimiento")
        _add_user("Información incompleta")

    # 4. Chequeo de Vigencia (NO afecta autenticidad; solo logging)
    vig_final = date_results.get("fecha_vigencia_final")
    if not vig_final or (isinstance(vig_final, str) and "Sugerida" in vig_final):
        score += 8
        _add_internal("⚠️ No se detectó vigencia válida")
    ...
    if not num_id:
        score += 6
        suspect = True
        _add_internal("⚠️ No se detectó número de identificación")
        _add_user("Información incompleta")
    elif num_id in ["123456789", "000000000", "111111111", "999999999"]:
        score += 40
        hard_fail = True
        _add_internal(f"⚠️ Número de ID claramente inválido: {num_id}")
    ...
    # 7. Consistencia adicional: nombre obviamente ficticio (evidencia fuerte)
    if nombre:
        ...
        if any(pattern in nombre_lower for pattern in fake_patterns):
            score += 35
            hard_fail = True
            _add_internal(f"⚠️ Nombre sospechoso: {nombre}")
            _add_user("Datos personales no válidos")

    # Escalamiento: múltiples inconsistencias fuertes => ALTO
    if not hard_fail and strong_flags >= 2:
        hard_fail = True
        _add_internal(f"⚠️ Escalamiento: {strong_flags} inconsistencias fuertes")

    # ===== Decisión final (semáforo) =====
    # ALTO: evidencia fuerte
    # MEDIO: sospecha / incertidumbre
    # BAJO: sin señales relevantes
    if hard_fail:
        riesgo = "alto"
        emoji = "🔴"
        color = "red"
        if not details_user:
            details_user = ["Alta probabilidad de falsificación"]
        _add_user("⚠️ VALIDACIÓN FÍSICA OBLIGATORIA - No confiar solo en análisis digital")
    elif suspect:
        riesgo = "medio"
        emoji = "🟡"
        color = "yellow"
        ...
        _add_user("⚠️ Se recomienda validación física del documento")
    else:
        riesgo = "bajo"
        emoji = "🟢"
        color = "green"
...
def update_main_ui_texts():
    """Actualiza todos los textos de la UI principal cuando cambia el idioma"""
    if not _I18N_OK:
        return
    
    try:
        # Actualizar header
        header_label.config(text=get_text('app_title'))
        
        # Actualizar botones de la barra
        btn_cargar.config(text=get_text('load_images'))
        btn_pegar.config(text=get_text('paste_clipboard'))
        btn_api.config(text=get_text('api_config'))
        
        # Actualizar título del panel
        title_ocr_label.config(text=get_text('result_ocr'))
        
        # Actualizar status
        status.config(text=get_text('drag_drop_hint'))
        
        # Actualizar botones del footer
        btn_analizar.config(text=get_text('analyze'))
        btn_ident.config(text=get_text('analyze_id'))
        btn_export.config(text=get_text('export'))
        btn_borrar.config(text=get_text('clear'))
        
        # Actualizar botón de toggle si existe y está visible
        try:
            if btn_toggle_original.winfo_exists():
                current_text = btn_toggle_original.cget('text')
                if 'original' in current_text.lower() or 'view' in current_text.lower():
                    btn_toggle_original.config(text=get_text('view_original'))
                else:
                    btn_toggle_original.config(text=get_text('view_translation'))
        except:
            pass
    except Exception as e:
        print(f"[i18n] Error updating UI texts: {e}")
...
def _format_ocr_text_with_normalized_dates(t: str, results: Dict[str, Optional[str]]):
    ...
    for key_prefix in ["vencimiento", "vence", "expiración", "expiracion", "vigencia", "valido hasta", "válido hasta", "valid thru", "valid through", "valid until", "caducidad", "fecha de caducidad", "expires", "expire", "expiry", "expiration", "date of expiry", "date of expiration"]:
