...
# ===========================
try:
    from devops_mcp import get_devops_mcp, DevOpsMCP
    _DEVOPS_MCP_OK = True
except Exception as e:
    print("DevOps MCP no disponible:", e)
    DevOpsMCP = None
    _DEVOPS_MCP_OK = False
...
def autenticar_con_keycloak():
    ...
    try:
        ...
        # Registramos el ingreso como "v4.6.2 (SSO)"
        try:
            registrar_ingreso(correo, nombre, "v4.6.2 (SSO)")
        except Exception as e:
            print("No se pudo registrar ingreso SSO:", e)

        # Inicializar DevOps MCP con el token de Keycloak
        if _DEVOPS_MCP_OK:
            try:
                token = keycloak_auth_instance.get_access_token()
                if token:
                    os.environ["KEYCLOAK_TOKEN"] = token
                    devops = get_devops_mcp(keycloak_token=token, gemini_api_key=GEMINI_API_KEY)
                    print("✅ DevOps MCP inicializado")
            except Exception as e:
                print(f"⚠️ No se pudo inicializar DevOps MCP: {e}")
...
SA_JSON_B64 = os.getenv("MAXIBOT_SA_BASE64", os.getenv("GOOGLE_CHATS_SA_BASE64", os.getenv("ATHENAS_SA_JSON_B64", ""))).strip()
...
def mostrar_api_key():
    ...
    def guardar_api_key():
        ...
        # Reinicializar DevOps MCP si el usuario tiene SSO y el token de Keycloak
        if _DEVOPS_MCP_OK and usuario_actual.get("sso") and keycloak_auth_instance:
            try:
                token = keycloak_auth_instance.get_access_token()
                if token:
                    devops = get_devops_mcp(keycloak_token=token, gemini_api_key=GEMINI_API_KEY)
                ...
                print(f"⚠️ No se pudo reinicializar DevOps MCP: {e}")
